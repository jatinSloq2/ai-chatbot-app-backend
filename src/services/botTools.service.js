...
const getSheetAuth = async (bot) => {
    ...
    const accessToken =
        cred.googleSheets.method === "oauth"
            ? await sheetsOauth.getValidAccessToken(cred) // refreshes + persists as needed
            : (await sheets.authFromCredential(cred.googleSheets)).accessToken;

    const auth = { accessToken, spreadsheetId: cred.googleSheets.spreadsheetId };
